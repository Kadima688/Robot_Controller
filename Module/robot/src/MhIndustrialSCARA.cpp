...
void Mh::MhIndustrialSCARA::setJointVelocity(const vpColVector &qdot){
    ...
    double delta_maxacc=1;
    std::vector<double> TargetVel(nDof);
    std::vector<double> TargetPos(nDof);
    std::vector<double> CurrPos(nDof);
    std::vector<double> OffsetPos(nDof);
    std::vector<double> EndVel(nDof);
    std::vector<double> MaxVel(nDof);
    std::vector<double> MaxAcc(nDof);
    std::vector<double> MaxJerk(nDof);
    //设置目标速度(单位为脉冲每毫秒)
    TargetVel[0]=jointvel[0]*1000; 
    TargetVel[1]=jointvel[1]*1000; 
    ...
    OffsetPos[2]=jointvel[2]*30; 
    OffsetPos[3]=jointvel[3]*30; 
    CurrPos[0]=Con2DemData.axisPos_scara.a1/RobotConfigData.pulseEquivalent[0];  
    CurrPos[1]=Con2DemData.axisPos_scara.a2/RobotConfigData.pulseEquivalent[1];
    CurrPos[2]=Con2DemData.axisPos_scara.d/RobotConfigData.pulseEquivalent[2];
    CurrPos[3]=Con2DemData.axisPos_scara.a4/RobotConfigData.pulseEquivalent[3];
    for(int i=0;i<nDof;i++){
        TargetPos[i]=CurrPos[i]+OffsetPos[i];
    }
    //设置最大速度(单位为脉冲每毫秒)
    MaxVel[0]=abs(M_PI*delta_maxvel*(180/PI)/RobotConfigData.pulseEquivalent[0]);
    MaxVel[1]=abs(M_PI*delta_maxvel*(180/PI)/RobotConfigData.pulseEquivalent[1]);
    MaxVel[2]=abs(M_PI*delta_maxvel*(180/PI)*z_lead / 360 / RobotConfigData.pulseEquivalent[2]);
    MaxVel[3]=abs(M_PI*delta_maxvel*(180/PI)/RobotConfigData.pulseEquivalent[3]);
    //设置最大加速度(单位为脉冲每2次方毫秒)
    MaxAcc[0]=abs(M_PI*delta_maxacc*(180/PI)/RobotConfigData.pulseEquivalent[0]);
    MaxAcc[1]=abs(M_PI*delta_maxacc*(180/PI)/RobotConfigData.pulseEquivalent[1]);
    ...
    case Mh::MhIndustrialRobot::STATE_POSITON_CONTROL:
        ...
        //     // std::cout<<"Targetvel:"<<TargetVel[0]<<"    "<<TargetVel[1]<<"    "<<TargetVel[2]<<"    "<<TargetVel[3]<<std::endl;
        //     // //输出最大速度
        //     // std::cout<<"Maxvel:"<<MaxVel[0]<<"    "<<MaxVel[1]<<"    "<<MaxVel[2]<<"    "<<MaxVel[3]<<std::endl;
        //     // //输出最大加速度
        //     // std::cout<<"MacAcc:"<<MaxAcc[0]<<"    "<<MaxAcc[1]<<"    "<<MaxAcc[2]<<"    "<<MaxAcc[3]<<std::endl;
        //     // //输出位置偏移量
        //     // std::cout<<"OffsetPose:"<<OffsetPos[0]<<"    "<<OffsetPos[1]<<"    "<<OffsetPos[2]<<"    "<<OffsetPos[3]<<std::endl;
        //     // //输出最大加加速度
        //     // std::cout<<"MacJerk:"<<MaxJerk[0]<<"    "<<MaxJerk[1]<<"    "<<MaxJerk[2]<<"    "<<MaxJerk[3]<<std::endl;
        //     controllerdata.motor.MC_GroupVisualServoMove(0,TRUE,OffsetPos,TargetVel,EndVel,MaxVel,MaxAcc,MaxJerk,true,count,Ruckig_Velocity_Control);
        //     // judge = 0 ;
        // }
    ...
    //首先将速度转换为角度
    // for(unsigned i=0;i<nDof;++i){
    //     //------------------这一部分是用MC_MoveRelative对应的代码
    //     double jointvel;
    //     jointvel=qdot[i]*(180/PI);//单位为度每秒
    //     double jointvelpulse;
    //     if(i == 2){
    //         jointvel = jointvel * z_lead / 360 ;
    //     }
    //     jointvelpulse=jointvel/RobotConfigData.pulseEquivalent[i]*0.001;//单位为脉冲每毫秒
    //     double jointpulse;
    //     jointpulse=jointvelpulse*30;//100ms这个轴应该移动得位移
