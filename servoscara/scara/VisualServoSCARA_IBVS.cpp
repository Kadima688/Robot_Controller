...
void RealsenseServoSCARA_IBVS(Mh::MhIndustrialSCARA *RobotSCARA,double opt_tagSize,bool adaptive_gain,bool opt_plot,bool opt_task_sequencing,bool opt_verbose,
                          double convergence_threshold_t,double convergence_threshold_tu,double convergence_threshold_image);
...
void VisualServoSCARA_IBVS(Mh::MhIndustrialSCARA* RobotSCARA){
  ...
  double convergence_threshold_t=0,convergence_threshold_tu=0,convergence_threshold_image=0.001;
  bool hasServo=0;
#ifdef USE_REALSENSE
  while(hasServo==0){
    if(RobotSCARA->ConChargeData.isEnable==1){
      sleep(1);
      RealsenseServoSCARA_IBVS(RobotSCARA,opt_tagSzie,adaptive_gain,opt_plot,opt_task_sequencing,opt_verbose,convergence_threshold_t,convergence_threshold_tu,convergence_threshold_image);
      hasServo=1;
    }
  }
#else
  ...
  */
  while(hasServo==0){
    if(RobotSCARA->ConChargeData.isEnable==1){
      sleep(1);
      MotorServoSCARA_IBVS(RobotSCARA,opt_tagSzie,adaptive_gain,opt_plot,opt_task_sequencing,opt_verbose,convergence_threshold_t,convergence_threshold_tu,convergence_threshold_image);
      hasServo=1;
    }
  }
#endif
